...
class DataIngestion():
    ...
    def initiate_data_ingestion(self): # I wil write the code here to read from the database
        logging.info("Entered the data ingestion method")
        try:
            df = pd.read_csv('notebook/data/StudentsPerformance.csv')
            logging.info("Exported/Read the dataset as dataframe")
            os.makedirs(os.path.dirname(self.ingestion_config.train_data_file_path), exist_ok=True)
...
if __name__ == "__main__":
    ...
    train_arr, test_arr,_ = data_transformation.initiate_data_transformation(train_data, test_data) # added train_arr, test_arr,_ = for model trainer implemenation
